...
//////////////////////////////////////////////////////////////////////////
void DisplayContext::SetView(IViewport* pView)
{
	view = pView;
	int width, height;
	RECT rect;
	::GetClientRect((HWND)pView->GetSafeHwnd(), &rect);
	m_width = float(rect.left-rect.right);
...
void DisplayContext::DrawDottedLine(const Vec3& p1, const Vec3& p2, float interval)
{
	...
	int totsegments =int(length / interval);
...
void DisplayContext::DrawLine2d(const CPoint& p1, const CPoint& p2, float z)
{
	...
	if (m_width == 0 || m_height == 0)
	{
		...
		if (view)
		{
			...
			view->Update();
			RECT rect;
			::GetClientRect((HWND)view->GetSafeHwnd(), &rect);
			m_width = float(rect.left - rect.right);
			m_height = float(rect.bottom - rect.top);
		}
	}
